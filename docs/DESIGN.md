...
| `src/EnginePerformance.tsx`, `usePerformance.ts`, `performance-format.ts` | The Performance section of an expanded engine card: one 1h/24h/7d window shared by all cards, loaded from `/api/v1/performance` only while Overview is shown and reloaded when any loaded request changes status or any engine's status or model list changes, never on the idle state poll (catalog check times are not part of that key). Each model row shows served and failed attempts, catalog listing, median and p95 first-chunk and non-stream times, and the median stream rate, marked ≈ when any sample used estimated tokens. |
...
`GET /api/v1/performance` reads request history for a window of 1 to 168 hours (history retention is seven days) and `summarize` groups every attempt by its engine and model. An attempt is served when it is the final attempt of a completed request. It failed when its transport failed, when the engine answered 408, 429 or 5xx, or when it was the final attempt of a request that ended in 502 after the engine answered (an interrupted stream or an oversized body). Cancellations and an engine's own 4xx answers count as neither. Timings come only from served attempts' `performance`, as nearest-rank medians and 95th percentiles, with stream and non-stream requests kept apart. `in_catalog` compares the model with the engine's current catalog and is null when that catalog is unknown, so a failed probe never reports a model as replaced. Rows are ordered by engine name, then most recently used model, which places a replacement directly above the model it replaced.
