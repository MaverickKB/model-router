...
The map contains saved permission policies and observed unassigned connections in its left column. Unassigned connections are grouped by direct source address, with individually selectable application or library observations. Source headings are presentation only; every observation keeps its own permission edges and details. A policy can be edited before traffic exists. An observed connection without a policy remains visible and shows the route gate it would encounter. Selecting a policy exposes its allowed routes and every recorded connection. Editing a route permission clearly applies to every connection using that policy.
